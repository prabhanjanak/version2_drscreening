...
async function main() {
  ...
  for (const row of rows) {
    ...
    for (let i = 0; i < uniqueParticipants.length; i++) {
      ...
      if (cleanName === pName) {
        ...
        if (cleanInst && pInst && cleanInst === pInst) {
          foundIdx = i;
          break;
        }
        // If one of them has no institution or unknown, or they match on something else
        if (!cleanInst || cleanInst === "unknown institution" || !pInst || pInst === "unknown institution") {
          foundIdx = i;
          break;
        }
      }
  ...
  console.log("Deduplicated total participants (Strategy 4):", uniqueParticipants.length);
}
