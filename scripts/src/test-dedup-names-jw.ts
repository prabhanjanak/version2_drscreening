...
async function main() {
  ...
  // Sweep name similarity threshold
  for (let nameTh = 0.8; nameTh <= 1.01; nameTh += 0.02) {
    ...
    for (const row of rows) {
      ...
      for (let i = 0; i < uniqueParticipants.length; i++) {
        ...
        const nameSim = getSimilarity(cleanName, pName);
        if (nameSim >= nameTh) {
          ...
          // Merge if email or mobile matches, OR if name is highly similar and institution matches exactly
    ...
    console.log(`Name Similarity Threshold ${nameTh.toFixed(2)} yields: ${uniqueParticipants.length} unique participants`);
  }
}
