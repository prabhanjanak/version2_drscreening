...
async function main() {
  ...
  for (const row of rows) {
    ...
    for (let i = 0; i < uniqueParticipants.length; i++) {
      ...
      if (cleanName === pName) {
        ...
        // If institution matches exactly (and not empty/unknown)
        if (cleanInst && pInst && cleanInst === pInst && cleanInst !== "unknown institution" && cleanInst !== "") {
          foundIdx = i;
          break;
        }
      }
  ...
  console.log("Count with no empty-inst merge fallback:", uniqueParticipants.length);
}
