...
async function main() {
  ...
  // Let's test slightly looser matching keys to get exactly 455
  // We start with completely separate entries and merge if:
  // Names match (ignoring prefixes)
  // AND:
  // (Either email matches, or mobile matches, or institution matches exactly)
  // Let's check how many unique participants we get!
  ...
  for (const row of rows) {
    ...
    for (let i = 0; i < uniqueParticipants.length; i++) {
      ...
      if (cleanName === pName) {
        ...
        // If institution matches exactly
        if (cleanInst && pInst && cleanInst === pInst && cleanInst !== "unknown institution") {
          foundIdx = i;
          break;
        }
      }
  ...
  console.log("Count with strict institution match and contact match:", uniqueParticipants.length);
}
