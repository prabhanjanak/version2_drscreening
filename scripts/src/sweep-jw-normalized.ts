...
async function main() {
  ...
  // Loop th fine
  for (let th = 0.95; th <= 1.005; th += 0.005) {
    ...
    for (const row of rows) {
      ...
      const normInst = normalizeInst(row.institution);
      ...
      for (let i = 0; i < uniqueParticipants.length; i++) {
        ...
        const pNormInst = normalizeInst(p.institution);
        ...
        if (cleanName === pName) {
          ...
          const sim = getSimilarity(normInst, pNormInst);
          if (sim >= th) {
            foundIdx = i;
            break;
          }
    ...
    console.log(`Threshold ${th.toFixed(3)} yields: ${uniqueParticipants.length} unique participants`);
  }
}
