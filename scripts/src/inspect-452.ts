...
async function main() {
  ...
  const merges: { target: RawRow; merged: RawRow }[] = [];
  ...
  for (const row of rows) {
    ...
    if (foundIdx === -1) {
      ...
      merges.push({ target: uniqueParticipants[foundIdx], merged: row });
  ...
  for (const m of merges) {
    console.log(`Merged: "${m.merged.name}" (${m.merged.sheet} Row ${m.merged.rowIdx}) into "${m.target.name}" (${m.target.sheet} Row ${m.target.rowIdx})`);
  }
}
