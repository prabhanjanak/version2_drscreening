...
async function run() {
  ...
  try {
    ...
    // 4. Update ALL existing patient registration Unique IDs to standard SEH/CAMPCODE/DDMMYYYY/SerialNumber format
    if (patientTableCheck.rows[0].exists) {
      const allPatientsRes = await client.query(`
        SELECT id, unique_id, date, serial_number, screening_place_code 
        FROM patients 
        ORDER BY date ASC, created_at ASC, id ASC;
      `);

      if (allPatientsRes.rows.length > 0) {
        console.log(`Checking and standardizing ${allPatientsRes.rows.length} existing patient records to SEH/CAMPCODE/DDMMYYYY/SerialNumber format...`);
        for (let i = 0; i < allPatientsRes.rows.length; i++) {
          const p = allPatientsRes.rows[i];
          const serialNum = i + 1;
          const padSerial = String(serialNum).padStart(4, '0');
          
          let campCode = (p.screening_place_code || '').toUpperCase().trim();
          if (!campCode) {
            const parts = (p.unique_id || '').split('/');
          ...
          const ddmmyyyy = `${dd || '08'}${mm || '08'}${yyyy || '2026'}`;
          const formattedId = `SEH/${campCode}/${ddmmyyyy}/${padSerial}`;
          
          await client.query(`
            UPDATE patients 
            SET unique_id = $1, serial_number = $2, screening_place_code = $3 
            WHERE id = $4;
          `, [formattedId, serialNum, campCode, p.id]);
        }
        console.log("All existing patient Unique IDs updated to SEH/CAMPCODE/DDMMYYYY/SerialNumber standard.");
      }
    }
